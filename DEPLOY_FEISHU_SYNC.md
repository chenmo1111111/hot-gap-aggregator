...
`gongkao_enriched.json`，秋招使用 `qiuzhao.json` 加上从公考源识别出的企业校园招聘。企业校招
从公考表移除后会进入秋招表；内部表只迁移 `来源` 以“自动”开头的行，手动行始终不动。

公开公考表字段为：`公告标题`（主字段）、`首次收录`、`类别`、`招聘人数`、`截止日期`、
...
飞书视图由用户手工建立，脚本只维护字段和数据，不创建或覆盖视图。使用当前以下 9 个业务视图。
`报名状态` 的“剩1天”到“剩5天”仍属于进行中，只是单独突出紧急度。
...
| 选调·本校可报 | `细分类别 = 选调生`；查看 `招录院校范围` 并搜索学校层次关键词 |
| 国企央企 | `细分类别 = 国企` 或 `央企` |
| 事业单位 | `细分类别 = 事业单位` |
| 银行 | `细分类别 = 银行` |
| 已结束 | `报名状态 = 已截止` |
...
不再同步到公开飞书表。
公开表只保留短期复盘窗口：有报名截止日期的自动行在截止后第 4 天删除；没有报名截止日期、
但有笔试日期的自动行在笔试后第 8 天删除。`来源=手动` 的行完全不受清理影响。“进行中”视图
隐藏已截止行，“已结束”视图显示尚在短期保留窗口内的已截止公告。

### 使用指南

Base 只保留 `📘 必看｜公考秋招资源库使用说明` 文档作为用户指南。旧 `使用说明` 数据表已删除，
且 `config/feishu_public_sync.yaml` 的 `instructions_table_enabled: false` 会阻止公开同步任务重新
创建或更新它。历史兼容函数仍保留；只有显式重新启用该开关时才会维护旧说明表。

### 仪表盘（网页端手动创建）

飞书开放 API 不能可靠创建和配置仪表盘。在 Base 左侧点击 `+` → `仪表盘`，数据源都选择公考表，
再建立以下图表：

| 图表 | 数据源与筛选 | 维度 | 度量 |
| --- | --- | --- | --- |
| 报名中总数 | 报名状态为 `报名中/剩1天/剩2天/剩3天/剩4天/剩5天` | 无 | 记录数 |
| 本周截止数 | 报名状态不为 `已截止`，距截止天数 `0–7` | 无 | 记录数 |
| 今日新增数 | 首次收录为今天 | 无 | 记录数 |
| 报名中·细分类别 | 同“报名中总数” | `细分类别` | 记录数；柱状图，降序 |
| 报名中·省份 Top 10 | 同“报名中总数” | `省份` | 记录数；柱状图，降序，只显示前 10 |
| 每日新增趋势 | 不额外筛选 | `首次收录`（按天） | 记录数；折线图，日期升序 |
| 报名状态占比 | 不额外筛选 | `报名状态` | 记录数；饼图，显示百分比 |

前三项选择“指标卡”，后四项分别选择表中指定图形。仪表盘完成后，把时间筛选器默认设为最近
30 天；报名中、本周截止两个指标不要套用该时间筛选器，以免漏掉较早发布但仍在报名的公告。

所有文本字段缺失时公开表显示 `/`；日期、数字、单选、复选框和超链接为空时仍保持空白。
比较数据时 `/` 与空白视为等价，首次升级会专门回填占位符，后续不会因此每天重复更新。

### 公考公开表升级后的一次性手动调整

首次运行新版同步后，脚本会自动新增 `首次收录`、`招录院校范围`，并删除旧的 `日期`、
`笔试科目`、`本校可报`。公考的日期依据统一使用 `首次收录`，报名时效使用 `截止日期`；秋招表的
`日期` 不受影响。飞书开放 API 不能可靠设置已有列的可视顺序及视图排序，请在飞书网页手动完成：

1. 在「全部信息」视图依次取消隐藏：`细分类别`、`省份`、`报名状态`、`截止日期`、`首次收录`、
   `学历要求`、`限户籍`、`招录院校范围`、`链接`、`备注`。
2. 隐藏 `同步ID` 及其它技术列；公开公考表不再保留 `来源` 列，把 `备注` 拖到最右侧。
   公开表手动新增的行不填 `同步ID`，同步、去重和过期清理都会绕开它。
3. 在「全部信息」点击「排序」，添加 `首次收录`，选择降序（最新日期在上）。
4. 「今日必做」仍按 `距截止天数` 升序；其它业务视图保留各自原排序，不要套用“全部信息”的排序。
5. 若升级前同步异常导致旧列仍存在，确认 `首次收录` 已有数据后，手动删除 `日期`、`笔试科目`、
   `本校可报`。

首次收录优先使用公告 `published_at`（粉笔正式公告对应 `issueTime`），只有公告发布日期缺失时
才回退到更新时间或报名开始时间。旧数据库
...
`首次收录 ｜ 类别 ｜ 公告标题 ｜ 招聘人数 ｜ 最低学历 ｜ 报名开始 ｜ 报名截止 ｜ 报名状态 ｜ 省份 ｜ 城市 ｜ 单位名称 ｜ 岗位性质 ｜ 限户籍 ｜ 限专业 ｜ 应届要求 ｜ 服务期 ｜ 招录院校范围 ｜ 备注 ｜ 链接`

公开表已删除 `来源` 列；`同步ID` 是需隐藏的技术列，有 ID 的行由同步程序管理，手动新增且无 ID 的行不参与自动清理。
`首次收录` 等于政府原文发布日期；省份统一去掉“省/市/自治区”后缀；城市
...
用户说明统一维护在 Base 的 `📘 必看｜公考秋招资源库使用说明` 文档中，定时同步只更新公考和
秋招数据表，不创建或更新旧说明数据表。
...
稳定 ID 首次出现日统计。`/etc/cron.d/hot-gap-daily-volume` 在每天 07:00（Asia/Shanghai）
播报前一自然日 `[00:00, 次日00:00)` 的完整数字；无论高低都发送，低于秋招 20 条或公考
3 条时在对应行附加检查提醒。当天后续 refresh 会继续记账，但同一统计日不会重复播报。
