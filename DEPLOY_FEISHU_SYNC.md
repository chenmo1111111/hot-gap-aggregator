...
公开表同步会在同一个 Base 中查找名为 `使用说明` 的数据表；不存在时通过 Bitable API 自动创建，
并维护 `视图 | 给谁看 | 怎么用` 三列。表中包含一条总说明和上述 9 个视图各一条说明。脚本只
更新这些固定说明，不删除用户另外添加的说明行。若应用缺少 Base 管理权限，创建会失败但公考、
秋招两张表仍各自完成同步；应确认应用拥有 `bitable:app` 且已作为可编辑文档应用加入该 Base。
