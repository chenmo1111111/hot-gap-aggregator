...
yingjie_requirement(应届要求，只能填'仅应届'/'应往届均可'/'往届可报'/'需核对岗位表'/'未明确'。只有公告明确所有岗位仅面向应届才填'仅应届'；明确所有岗位应往届都可报才填'应往届均可'；至少一个岗位明确允许非应届毕业生报名填'往届可报'；不同岗位要求不同或仅有部分岗位的应届限制证据填'需核对岗位表'；未写清楚填'未明确'，不能从没写限制推断为不限)，
...
NON_PORTAL_HOST_SUFFIXES = (
    ...
    "sohu.com", "163.com", "baidu.com", "bing.com", "offcn.com", "nowcoder.com",
    "wondercv.com", "yingjiesheng.com", "niuqizp.com",
)
...
def _search_apply_url(
    ...
    try:
        ...
        return None
    if result:
        stats["apply_url_search_found"] += 1
    return result
