...
- `config/yingjiesheng.yaml` 维护应届生可投递职位的关键词、城市、类型和时间窗；宣讲会、双选会和招聘会统一过滤。该源由国内服务器采集，主站被 WAF 拦截时按 `fallback_source` 尝试海投校招，失败会保留上一次 sidecar 数据。
...
小红书规则监控每天读取“规则修订 / 规则新增 / 意见征集”列表，以 `(rule_id, 发布日期)` 作为已通知台账。首次启用只建立基线；之后对上次成功运行以来（额外回看 2 天）新发布且未通知的规则抓正文、刷新在售商品、读取规则所附腾讯文档、执行 AI 影响分析，并无论分析结论如何同时写入网站预警和发送飞书。腾讯文档需要登录时使用独立的 `TENCENT_DOC_COOKIE_JSON`；Cookie 和外链访问参数不会写入日志或交给模型。外链读取失败会保留旧缓存并降级为人工核对，不阻断新规则通知。HTTP 日志会自动隐藏飞书 Webhook、Bark 地址和 API Key。若 Webhook 曾出现在旧日志、截图或聊天记录中，应立即在飞书机器人设置中重新生成，并只把新值写入服务器 `.env`；不要提交到仓库。
