...
def test_merge_keeps_base_first_and_skips_sheet_duplicates() -> None:
    ...
    assert [item["extra"]["id"] for item in output["items"]] == [
        "base-1", "sheet-duplicate-url", "sheet-new",
    ]
    assert output["status"]["base_item_count"] == 1
    assert output["status"]["sheet_added_count"] == 2
    assert output["status"]["sheet_duplicate_count"] == 1
    assert output["status"]["sheet_merged_count"] == 1
    assert output["items"][0]["url"] == "https://a.test/1"
    assert output["items"][0]["extra"]["backup_urls"] == ["https://b.test/2"]
    assert [item["rank"] for item in output["items"]] == [1, 2, 3]
...
def test_merge_adds_server_watchers_before_sheet_and_assigns_stable_id() -> None:
    ...
    assert output["status"]["sheet_duplicate_count"] == 0
    assert output["status"]["sheet_merged_count"] == 0
    assert watcher["url"] == "https://rst.ln.gov.cn/xuandiao/1"
...
def test_sheet_official_url_overrides_duplicate_fenbi_links() -> None:
    ...
    assert set(output["items"][0]["extra"]["backup_urls"]) == {
...
def test_government_link_is_not_overwritten_by_sheet_or_watcher_snapshot() -> None:
    ...
    assert output["status"]["sheet_merged_count"] == 1
    assert output["items"][0]["extra"]["backup_urls"] == [
        "https://mp.weixin.qq.com/s/old-sheet-copy",
    ]
