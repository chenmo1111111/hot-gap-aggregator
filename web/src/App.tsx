...
const sourceNames: Record<string, string> = {
  ...
  nowcoder: '牛客', qiuzhao: '秋招', alerts: '预警',
};
...
function GongkaoView({ items, sites }: { items: Item[]; sites: OfficialSite[] }) {
  ...
  return <div className="grid gap-8">
    <section className="rounded-2xl border border-[var(--line)] bg-[var(--card)] p-4"><div className="mb-3 flex flex-wrap items-center gap-2"><b className="mr-2 text-sm">省份多选</b>{allProvinces.map((province) => <button key={province} onClick={() => toggle(province)} className={`rounded-full px-3 py-1.5 text-xs font-bold ${provinces.includes(province) ? 'bg-cyan-500 text-white' : 'bg-[var(--soft)]'}`}>{province}</button>)}{provinces.length > 0 && <button onClick={() => setProvinces([])} className="text-xs text-cyan-600">清空</button>}</div><div className="flex flex-wrap items-center gap-2"><b className="mr-2 text-sm">考试类型</b>{['国考', '选调生', '省考'].map((type) => <button key={type} onClick={() => setExamType((current) => current === type ? 'all' : type)} className={`rounded-full px-3 py-1.5 text-xs font-bold ${examType === type ? 'bg-[var(--ink)] text-[var(--paper)]' : 'bg-[var(--soft)]'}`}>{type}</button>)}<select aria-label="考试类型" value={examType} onChange={(event) => setExamType(event.target.value)} className="rounded-lg border border-[var(--line)] bg-[var(--paper)] px-3 py-2 text-sm font-normal"><option value="all">全部类型</option>{allTypes.map((type) => <option key={type}>{type}</option>)}</select></div></section>
    {Object.entries(groups).map(([title, entries]) => <section key={title}><h2 className="mb-3 text-xl font-black">{title} <span className="font-mono text-xs text-[var(--muted)]">{entries.length}</span></h2><div className="grid gap-3">{entries.map((item) => { const left = daysFromNow(item.extra?.endSignUpTime); const universityHits = Array.isArray(item.extra?.target_university_hit) ? item.extra.target_university_hit.map(String) : []; const cityHits = Array.isArray(item.extra?.city_focus_hit) ? item.extra.city_focus_hit.map(String) : []; return <button key={item.url} onClick={() => openItem(item)} className={`grid gap-3 rounded-2xl border bg-[var(--card)] p-4 text-left hover:border-cyan-400 sm:grid-cols-[auto_1fr_auto] sm:items-center ${universityHits.length ? 'border-rose-400 ring-1 ring-rose-300' : cityHits.length ? 'border-amber-400 ring-1 ring-amber-300' : 'border-[var(--line)]'}`}><span className="w-fit rounded-lg bg-cyan-100 px-3 py-2 text-xs font-black text-cyan-900">{String(item.extra?.province || '全国')}</span><span><span className="mb-1 flex flex-wrap items-center gap-2"><b>{item.title_zh || item.title}</b>{item.is_new && <i className="not-italic rounded bg-lime-300 px-1.5 text-[10px] font-black text-slate-950">新</i>}{cityHits.map((name) => <i key={name} className="not-italic rounded bg-amber-300 px-2 py-0.5 text-[10px] font-black text-amber-950">重点城市 · {name}</i>)}{universityHits.map((name) => <i key={name} className="not-italic rounded bg-rose-500 px-2 py-0.5 text-[10px] font-black text-white">定向 · {name}</i>)}{item.extra?.subsource === 'scs' && <i className="not-italic rounded bg-red-100 px-2 py-0.5 text-[10px] font-black text-red-700">国家公务员局</i>}</span><small className="text-[var(--muted)]">{String(item.extra?.exam_type || '其他')} · 报名 {md(item.extra?.startSignUpTime)}～{md(item.extra?.endSignUpTime)} · 笔试 {md(item.extra?.startWriteTime)}</small></span>{left !== null && left >= 0 && <span className={`text-sm font-black ${left <= 2 ? 'text-rose-500' : 'text-orange-500'}`}>距截止 {left} 天</span>}</button>; })}{entries.length === 0 && <p className="rounded-xl border border-dashed border-[var(--line)] p-4 text-xs text-[var(--muted)]">当前筛选下暂无项目</p>}</div></section>)}
...
function AlertsView({ feed }: { feed: AlertFeed }) {
  return <section className="mx-auto max-w-4xl">
    <div className="mb-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-950 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-100"><b>人才政策预警</b><p className="mt-1 text-xs opacity-70">百度云每 12 小时监听目标地区人社局公告与核心补贴政策页。</p></div>
...
function App() {
  ...
  const [alertsLastSeen, setAlertsLastSeen] = useState(() => localStorage.getItem('alerts_last_seen') || '');
  ...
  useEffect(() => { Promise.all([
    ...
    fetch('./data/alerts.json', { cache: 'no-store' }).then((r) => r.json()).catch(() => ({ generated_at: '', items: [] })),
  ]).then(([nextFeed, nextTrends, nextSites, nextAlerts]) => { setFeed(nextFeed); setTrends(nextTrends); setSites(nextSites.sites); setAlerts(nextAlerts); }).catch(() => setFeed({ generated_at: '', items: [] })); }, []);
  ...
  useEffect(() => {
    if (active !== 'alerts' || !latestAlert) return;
    localStorage.setItem('alerts_last_seen', latestAlert);
    setAlertsLastSeen(latestAlert);
  }, [active, latestAlert]);
  useEffect(() => {
