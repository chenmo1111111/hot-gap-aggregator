...
const sourceNames: Record<string, string> = {
  ...
  telegram: 'Telegram', gongkao: '公考', xiaohongshu: '小红书雷达', papers: '顶刊', qiuzhao: '秋招',
};
  ...
  return <div className="grid gap-8 lg:grid-cols-2">
    {sections.map(([key, title, subtitle]) => <section key={key}>
      ...
      <div className="grid gap-2">{(trends?.[key] ?? []).map((item) => <button key={`${item.source}-${item.url}`} onClick={() => openItem(item)} className="grid grid-cols-[32px_1fr_auto] items-center gap-3 rounded-xl border border-[var(--line)] bg-[var(--card)] p-3 text-left hover:border-cyan-400">
        <span className="font-mono text-sm text-[var(--rank)]">{String(item.rank).padStart(2, '0')}</span><span className="min-w-0"><b className="block truncate text-sm">{item.title_zh || item.title}</b><small className="text-[var(--muted)]">{sourceNames[item.source] ?? item.source}{key === 'longest_on_board' ? ` · ${item.days_on_board ?? 1} 天` : ''}</small></span><Sparkline history={item.rank_history} />
      </button>)}</div>
    </section>)}
  </div>;
}
...
function GongkaoView({ items, sites }: { items: Item[]; sites: OfficialSite[] }) {
  ...
  const allTypes = useMemo(() => Array.from(new Set(items.map((item) => String(item.extra?.exam_type || '其他')))).sort(), [items]);
  ...
  return <div className="grid gap-8">
    <section className="rounded-2xl border border-[var(--line)] bg-[var(--card)] p-4"><div className="mb-3 flex flex-wrap items-center gap-2"><b className="mr-2 text-sm">省份多选</b>{allProvinces.map((province) => <button key={province} onClick={() => toggle(province)} className={`rounded-full px-3 py-1.5 text-xs font-bold ${provinces.includes(province) ? 'bg-cyan-500 text-white' : 'bg-[var(--soft)]'}`}>{province}</button>)}{provinces.length > 0 && <button onClick={() => setProvinces([])} className="text-xs text-cyan-600">清空</button>}</div><label className="text-sm font-bold">考试类型 <select value={examType} onChange={(event) => setExamType(event.target.value)} className="ml-2 rounded-lg border border-[var(--line)] bg-[var(--paper)] px-3 py-2 font-normal"><option value="all">全部类型</option>{allTypes.map((type) => <option key={type}>{type}</option>)}</select></label></section>
    {Object.entries(groups).map(([title, entries]) => <section key={title}><h2 className="mb-3 text-xl font-black">{title} <span className="font-mono text-xs text-[var(--muted)]">{entries.length}</span></h2><div className="grid gap-3">{entries.map((item) => { const left = daysFromNow(item.extra?.endSignUpTime); return <button key={item.url} onClick={() => openItem(item)} className="grid gap-3 rounded-2xl border border-[var(--line)] bg-[var(--card)] p-4 text-left hover:border-cyan-400 sm:grid-cols-[auto_1fr_auto] sm:items-center"><span className="w-fit rounded-lg bg-cyan-100 px-3 py-2 text-xs font-black text-cyan-900">{String(item.extra?.province || '全国')}</span><span><span className="mb-1 flex flex-wrap items-center gap-2"><b>{item.title_zh || item.title}</b>{item.is_new && <i className="not-italic rounded bg-lime-300 px-1.5 text-[10px] font-black text-slate-950">新</i>}</span><small className="text-[var(--muted)]">{String(item.extra?.exam_type || '其他')} · 报名 {md(item.extra?.startSignUpTime)}～{md(item.extra?.endSignUpTime)} · 笔试 {md(item.extra?.startWriteTime)}</small></span>{left !== null && left >= 0 && <span className={`text-sm font-black ${left <= 2 ? 'text-rose-500' : 'text-orange-500'}`}>距截止 {left} 天</span>}</button>; })}{entries.length === 0 && <p className="rounded-xl border border-dashed border-[var(--line)] p-4 text-xs text-[var(--muted)]">当前筛选下暂无项目</p>}</div></section>)}
