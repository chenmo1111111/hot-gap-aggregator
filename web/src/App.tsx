...
function App() {
  ...
  useEffect(() => { Promise.all([
    ...
    fetch('./data/job_quicklinks.json').then((r) => r.json()).catch(() => ({ items: [] })),
  ]).then(([nextFeed, nextTrends, nextSites, nextAlerts, nextJobLinks]) => { setFeed(nextFeed); setTrends(nextTrends); setSites(nextSites.sites); setAlerts(nextAlerts); setJobQuicklinks(nextJobLinks.items); }).catch(() => setFeed({ generated_at: '', items: [] })); }, []);
