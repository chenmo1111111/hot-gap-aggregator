...
describe('authenticated app bootstrap', () => {
  ...
  it('shows private mailbox deadlines only to admins and removes completed rows', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === '/api/admin/mail-deadlines/preview-text' && init?.method === 'POST') return json({ title: '伯特利面试-张利杰', type: '面试', start_at: '2026-09-23T17:30:00+08:00', deadline_at: null, action_url: 'https://meeting.dingtalk.com/j/test', summary: '17:30参加钉钉面试', confidence: 'high', source_url: '' });
    ...
    fireEvent.change(screen.getByLabelText('邀请文字'), { target: { value: '张瑞邀请你参加钉钉会议\n主题：伯特利面试-张利杰' } });
    fireEvent.click(screen.getByRole('button', { name: '识别文字' }));
    expect(await screen.findByDisplayValue('伯特利面试-张利杰')).toBeInTheDocument();
    expect(screen.getByLabelText('开始时间')).toHaveValue('2026-09-23T17:30');
    ...
    expect(JSON.parse(String(createCall?.[1]?.body))).toMatchObject({ title: '伯特利面试-张利杰', type: '面试', deadline_at: null });
